...
5. **Use Case-Aware**: Automatic algorithm selection by use case
...
let scheme = CryptoSelector::select_encryption_scheme(data, &config, None)?;
println!("Selected scheme: {:?}", scheme);

// Force specific use case
let scheme = CryptoSelector::select_encryption_scheme(
    data,
    &config,
...
Hardware type definitions only. This module provides the `CpuFeatures` struct for representing CPU capabilities. No detection or routing logic is included in the Apache edition.

> **Note**: Hardware detection, adaptive routing, and performance-based algorithm selection are available in the enterprise `arc-enterprise-perf` crate. The underlying crypto library (`aws-lc-rs`) handles AES-NI and SIMD acceleration automatically at the C level.

**Public Types:**
- `CpuFeatures` - CPU feature flags (informational)
...
- CPU AVX2/SIMD: faster for hash operations and PQC

No application-level configuration is needed. Build with `--target-cpu=native` to enable all available CPU features.
